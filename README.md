...
主监控视频在 **warning / alarm** 阶段叠加一个**固定位置、固定尺寸**的
YOLO 风格异常区域框，用来表示视觉模型已确认物料堆积异常区域：

- 无框 = 视觉尚未确认异常区域；有框 = 视觉已确认异常区域
- 边框颜色随等级变化：warning 橙色、alarm 红色；标签形如 `物料堆积 0.92`
- 框位置与尺寸在 warning / alarm 完全相同，**不做跟踪、不缩放、不移动**
- 该框同时出现在**综合监控**与**雷视联动**页面（由统一上下文派生，保证一致）

视觉检测到异常后会生成**带框证据图**（`frontend/public/images/evidence/`），
作为异常取证图接入**报警详情**与**数据溯源**，供现场人员复核处置。

> 说明：这是**展示层**的视觉异常表达，不是真实在线推理 ——
> 不运行 YOLO 推理、不加载模型权重、不引入推理依赖。
> 框的位置由人工查看监控视频后固定标定。
> 证据图由 `scripts/generate_evidence.py` 生成，坐标直接读取后端配置，
> 因此网页上的框与证据图上的框完全一致。
...
| AI 模型服务 | OpenAI-compatible 客户端（标准库实现） | 可接本地 llama.cpp 或任意兼容端点 |
...
平台支持接入大语言模型，用于**预警与报警之后的辅助分析**。

| 项 | 说明 |
|---|---|
| 支持的服务 | 本地 **llama.cpp** server，以及任何提供 OpenAI 兼容 `/v1/chat/completions` 的服务 |
| 触发时机 | 视频同步进入 **预警 / 报警** 阶段时，以及查看历史报警时按需分析 |
| 输入 | 当前监测数据 + 从历史知识库检索出的 Top-3 相似案例 |
| 输出 | 情况摘要、可能原因、建议检查、建议处置、参考历史事件 |

**模型不参与底层报警规则判定。** 风险等级始终由雷达、视觉与联合判定规则决定；
模型只回答「报警发生以后如何理解和处理」。即使模型服务完全不可用，
平台的监测、预警、报警、数据溯源、知识库功能全部照常工作。

视频每约 20 秒循环一次，因此**同一阶段的重复进入会复用后端缓存**，
不会因为循环而反复调用模型。

模型配置在「系统设置 → AI 模型服务」中填写，**默认不启用**。
所有模型请求都由后端发出，API Key 不下发浏览器。
