...
**Commit**：`d780b6926ffb87534a6cb4205df181ab42e6a11f`
（`feat: give cameras 03 and 04 their own footage` → 已推送）

---

## 轮次 22 — 移除未实现能力的描述

**背景**：用户要求把智能预警板块的「扩展能力与技术路线」删掉，
并明确原则：**没有实现的就不要写出来**。

**该判断成立**：该板块列的三项能力平台都没有实现 ——
杂质检测与自动分拣（含机械臂联动）、LSTM 时序预测与多目标跟踪、
跨行业推广。虽然当时标注了「规划中」，但把未实现的能力写进交付界面
本身就是负担，也容易让看的人误以为已有相应能力。

**清理范围**（顺带排查同类内容，共 2 处 + 1 处措辞）

| 位置 | 处理 |
|---|---|
| 智能预警页「扩展能力与技术路线」面板 | **整块删除**（3 张规划能力卡） |
| 系统设置页「规划能力」小节 | **整块删除**（3 张规划中卡片） |
| 系统设置页「异常报警与推送」卡片 | 去掉「原系统经钉钉机器人通知相关人员」—— 平台并未接入钉钉；并删除该句与本轮无关的表述 |
| 系统设置页面板标题 | 「系统能力与技术路线」→「系统能力」 |

**同时清理**：随之不再使用的样式
`.predict__caps` / `.predict__cap*` / `.settings__cap--planned`
以及对应的响应式规则。

**保留**：系统设置里的「已部署能力」三张卡（雷视联合判断、异常报警、
数据存储与溯源）—— 这些都是真实已实现并可在线验证的能力。

**验证**

| 检查项 | 结果 |
|---|---|
| 前端 tsc + vite build | 通过（632 模块，输出体积由 104.81 KB 降至 101.72 KB） |
| 产物复查（应移除） | `扩展能力`/`规划能力`/`规划中`/`技术路线`/`LSTM`/`多目标跟踪`/`杂质检测`/`自动分拣`/`机械臂`/`跨行业`/`食品加工`/`钉钉`/`predict__cap` 在 JS 与 CSS 中**均为 0 处** |
| 产物复查（应保留） | `已部署能力`×1、`雷视联合判断`×1、`数据存储与溯源`×1、`系统能力`×2 |
| 资源与接口 | 首页、四段视频、`/api/prediction`、`/api/realtime`、证据图均 200 |
| 双入口 | 主入口与 `/smoking/` 均 200 |
| 已有站点回归 | fitness(80) 200 |

**Commit**：`e3772d2cf451a25be98b51c2644f287a2642ba45`
（`fix: drop descriptions of unimplemented capabilities` → 已推送）

---

## 轮次 23 — 移除说明性质的文案

**背景**：用户要求删掉系统设置「运行模式」下的提示
（"若现场监控视频暂不可用，平台会自动回退到静态监控画面……"），
并明确原则：**这种说明性质的不需要，做出来他们就能够看得懂**。

**该判断成立**：这类文字是在解释"我们的实现是怎么兜底的"，
属于开发视角的说明，而不是使用者需要的信息。功能做出来自然就懂，
写出来反而显得心虚。这与轮次 22 的原则一脉相承 ——
**界面上只留使用者需要的信息**。
...
| 位置 | 原内容 | 处理 |
|---|---|---|
| 系统设置「运行模式」 | 「提示：若现场监控视频暂不可用……不会出现空白或报错」 | **删除** |
| 数据溯源页「溯源说明」面板 | 三段：本页对应原系统的历史查询能力……／每条记录都保留处理过程记录……／报警内容口径：…… | **整块删除** |
| 雷视联动页「为什么需要融合」 | 脚注「注：融合的目标不是用大模型替代传感器……」 | **删除** |

同时清理随之不再使用的样式
`.settings__mode-note`、`.trace__note*`、`.fusion__compare-foot`
以及 `TracePage` 中不再使用的 `IconDevice` 导入。

**保留**：各面板的**字段级 hint**（如最小测距的说明）与
「为什么需要融合」的对比表本身 —— 那些是运行信息与能力对比，
不是"解释我们怎么实现"的说明文字。
...
| 检查项 | 结果 |
|---|---|
| 前端 tsc + vite build | 通过（632 模块；产物由 101.72 KB 降至 **100.69 KB**，CSS 由 59.87 降至 59.17 KB） |
| 产物复查（应移除） | `若现场监控视频暂不可用`/`不会出现空白或报错`/`溯源说明`/`本页对应原系统的历史查询能力`/`报警内容口径`/`注：融合的目标` 均 **0 处** |
| 残留样式复查 | `settings__mode-note`/`trace__note`/`fusion__compare-foot`/`predict__cap` 在 JS 与 CSS 中**均为 0 处** |
| 产物复查（应保留） | `运行模式`×1、`已部署能力`×1、`为什么需要融合`×1、`30 分钟风险预测`×1 |
| 资源与接口 | 首页、四段视频、`/api/realtime`、`/api/alarms`、`/api/knowledge`、证据图均 200 |
| 双入口 | 主入口与 `/smoking/` 均 200 |
| 已有站点回归 | fitness(80) 200 |
...
**Commit**：`9bd8798200bd30a2990cacddd44e5d0236f4ad9e`
（`feat: use the final monitor footage with an overlay camera label` → 已推送）

---

## 轮次 25 — 精简预测免责声明

**背景**：用户要求智能预警页的免责声明只保留
「预测为概率性判断，存在不确定性」，**不要句号**，其余删掉。
...
删掉的后半句属于"解释我们不会做什么"，与轮次 22/23 的原则一致 ——
界面上只留使用者需要的信息。前半句是必要的能力边界说明，保留。

**顺带确认**：「报警内容口径：设备名称 + 设备 IP + 报警时间 + 距离信息」
这四项字段**已经在自然位置呈现**，无需另找地方补：

| 字段 | 所在位置 |
|---|---|
| 报警时间 / 设备名称 / 设备 IP | 报警详情弹层「事件基本信息」（两列字段表） |
| 距离信息 | 同弹层的「雷达测距」字段 + 「当时监控画面与数据走向」趋势图 |
| 四项全部 | 数据溯源页「历史事件明细」表格的独立列（时间 / 设备 / 设备 IP / 测距） |

因此该句在轮次 23 删除后没有信息缺失。
...
|---|---|
| 前端 tsc + vite build | 通过（632 模块，无警告） |
| 产物复查 | 免责声明为「预测为概率性判断，存在不确定性」；`不会给出`/`一定堵料`/`确定性结论` 均 **0 处** |
| 四项字段仍在 | `设备名称`×3、`设备 IP`×5、`报警时间`×3、`雷达测距`×13 |
| 资源与接口 | 四段视频、`/api/realtime`、`/api/prediction`、`/api/alarms`、`/api/knowledge`、证据图均 200 |
| 双入口 | 主入口与 `/smoking/` 均 200 |
| 已有站点回归 | fitness(80) 200 |

**Commit**：`f7e90a064884dfa0966623a562b0b17aad14020c`
（`fix: shorten the prediction disclaimer` → 已推送）

---

## 轮次 26 — 修复「角标有数字但当前报警为空」

**现象**：侧栏「异常报警」角标显示 **1**，点进去「当前报警」却是空的。
...
| 当前报警页 | `alarms.filter(status === pending \|\| processing)` | **未处置**的报警记录 |

实测：6 条报警记录**全部是 `archived`（已归档）**，未处置数为 0；
而 `today_warnings = 1`（今天有 1 条预警级记录）。
两个数字各自都对，但指向的不是同一件事 —— 点进去必然为空。

**修复**

1. **角标改为与页面同口径**：未处置记录数 + 当前画面的临时报警数，
   为 0 时不显示。角标与列表从此不可能不一致。
2. **「当前报警」补上实时派生的临时报警**（按用户确认的预期）：
   画面进入预警/异常阶段时，由当前遥测派生一条报警显示在列表顶部。

**临时报警的性质**（关键约束）

- 由 `buildCurrentAlarm(telemetry, trend)` 在**内存中派生**，
  **不写入任何存储** —— 视频每约 20 秒循环一次，落库会让历史记录失去可信度
- 画面回到 normal / attention 阶段时**自然消失**
- 与首页、雷视联动、侧栏角标**同源**（都来自 `VideoSyncContext`），
  因此侧栏数字、列表条数、详情内容必然一致
- 点击查看详情时直接用派生好的对象，**不走 `/api/alarms/{id}`**
  （该编号在后端并不存在）；报警编号用 `LIVE-<t>` 前缀以便区分

**其余修正**

- 「当前报警」空状态文案：「系统运行正常。报警产生后会自动出现在这里」→
  「所有报警均已处置并归档，可在「历史报警」中查看完整记录」——
  原文案在"有历史记录但都已归档"时容易让人以为功能有问题

**验证**

| 检查项 | 结果 |
|---|---|
| 后端 pytest | **220 passed**（无回归） |
| 前端 tsc + vite build | 通过（632 模块，无警告） |
| 派生逻辑自检 | `node scripts/check-live-alarm.mjs`：t=0~5.5 无临时报警，t=6~8.5 预警级，t=9~10 严重级；等级、状态、处理过程、证据图、趋势点全部断言通过 |
| 公网产物 | 含 `LIVE-` 编号前缀与新的空状态文案 |
| 角标口径 | 当前未处置为 0 → 角标**不显示**（与页面一致） |
| 接口与资源 | `/api/alarms`、`/api/system/status`、`/api/realtime`、四段视频均 200 |
| 双入口 | 主入口与 `/smoking/` 均 200 |
| 已有站点回归 | fitness(80) 200 |

**新增**：`frontend/scripts/check-live-alarm.mjs`（派生逻辑自检）。

**Commit**：`5e26822d89f0529cc8f2ab6512cbf8bfc23e2005`
（`fix: align the alarm badge with the current-alarm list` → 已推送）
...
四个机位都已接入画面，但面板仍写着「已接入画面 1 路」，
且「监控点总数 7 个」在本页（只展示 4 路视频）语境下没有意义。

| 项 | 原值 | 现值 |
|---|---|---|
| 监控点总数 | 7 个（与本页无关，删除） | — |
| 已接入画面 | **1 路**（写死） | **4 路**（由 `POINTS.length` 计算） |
| 其中带异常检测 | 无此项 | **3 路**（由 `kind === '雷达 + 视觉'` 计算） |

改为运行时计算而非写死，将来增删机位不会再次出现数据与实际不符。

**2. 侧栏页脚「数据实时刷新 · 2026」整行删除**（含前面的小圆点）
—— 属于状态装饰，不是使用者需要的信息。同时清理 `.sidebar__version`
与 `.sidebar__version-dot` 样式及响应式规则中的引用。

**3. 智能预警「建议措施」里的说明性条目删除**

原文：`说明：画面同步模式的时间轴经过压缩，用于呈现真实系统中可能跨越
更长时间发生的趋势；预测表达的是"在当前趋势持续条件下"的风险走向。`

这正是用户在轮次 23 定下的原则要清理的对象 —— 解释实现方式，而非给出建议。

**4. 系统设置「运行模式 → 画面同步」的描述收尾**

去掉末尾「为便于现场连续展示，时间轴经过压缩，用于呈现真实系统中可能
跨越更长时间发生的趋势」，只保留能力说明本身。

**验证**

| 检查项 | 结果 |
|---|---|
| 前端 tsc + vite build | 通过（632 模块，无警告；JS 103.60→103.28 KB，CSS 59.26→58.98 KB） |
| 产物复查（应移除） | `数据实时刷新`/`sidebar__version`/`时间轴经过压缩`/`监控点总数` 在 JS 与 CSS 中**均为 0 处** |
| 计算值核对 | `POINTS` 四项 `hasStream: true` → 4 路；`kind === '雷达 + 视觉'` 三项 → 3 路 |
| 产物复查（应保留） | `已接入画面`×1、`其中带异常检测`×1；免责声明仍为「预测为概率性判断，存在不确定性」 |
| 接口与资源 | `/api/realtime`、`/api/monitor-points`、四段视频、证据图均 200 |
| 双入口 | 主入口与 `/smoking/` 均 200 |
| 已有站点回归 | fitness(80) 200 |

**Commit**：`f5565a27db29cb6ee55a5664d46c3a8190ea354f`
（`fix: correct the monitor overview counts and drop more filler copy` → 已推送）

---

## 轮次 28 — 说明性内容清理与参赛信息脱敏

用户一次列出九项，合并处理。核心原则不变：
**界面上只留使用者需要的信息；解释实现方式、说明"我们不会做什么"的一律删除。**

**1. 雷视联动「传感器规格」的说明段删除**

原文解释"设备最高刷新能力 1000 Hz 与实际采集频率 10 Hz 不是一回事"。
规格表已列出两个数值，无需再解释它们的关系。

**2. 雷视联动「为什么需要融合」整个面板删除**

三方对比（视觉 AI / 雷达 / 融合判断）共 14 条正负项 ——
属于技术论证材料，不是运行界面该有的内容。

**3. 雷视联动「视觉 AI」的概率模型说明段删除**

原文"视觉结果属于概率模型输出，会受光照、遮挡与粉尘影响……"。
置信度字段本身已带 hint「概率模型输出，不是准确率」，无需重复论证。

**4. 侧栏「知识库」副标题**

`历史异常处理经验库` → `历史异常事件与处理记录`

**5. 系统设置「检测任务控制」的最后一条说明删除**

原文"页面按钮不会、也不应该被理解为可以关闭真实生产设备"。
保留了面板自身的 description「仅控制检测任务，不控制真实生产设备」——
那是**按钮的作用范围**，属于必要的操作信息，不是解释性文字。

**6. 系统设置「运行模式」的提示段删除**

该段在轮次 23 已删过一次，此处确认为同一处，已无残留。

**7. 参赛脱敏：厂商信息不出现具体单位**

`重邮自研（重庆邮电大学）` → **`XX 大学自研`**
比赛需要避嫌，不出现具体高校与工厂名称。全站已确认无其它残留。

**8. 视频监控：去掉缩略卡上的判断模式徽标**

Camera 02/03/04 的「联合判断 / 单独判断」徽标删除，
`PointSlot` 的 `mode` 与 `kind` 两个字段一并从数据中移除。
监控点概览的「其中带异常检测」改为引用常量 `RADAR_VISION_COUNT`。

**同时清理**：随之不再使用的样式
`.fusion__spec-note` / `.fusion__probabilistic` / `.fusion__compare*`
（含响应式规则）与 `IconInfo` 导入。

**验证**

| 检查项 | 结果 |
|---|---|
| 前端 tsc + vite build | 通过（632 模块，无警告；JS 103.28→**100.20 KB**，CSS 58.98→**57.42 KB**） |
| 产物复查（应移除） | 九项文案 + 三个样式类在 JS 与 CSS 中**均为 0 处**（逐项列出见部署输出） |
| 脱敏复查 | `重邮自研`/`重庆邮电大学` 全站 0 处；显示为 `XX 大学自研` |
| 产物复查（应保留） | `已接入画面`×1、`其中带异常检测`×1、`仅控制检测任务`×1 |
| 接口与资源 | `/api/realtime`、`/api/knowledge`、四段视频、证据图均 200 |
| 双入口 | 主入口与 `/smoking/` 均 200 |
| 已有站点回归 | fitness(80) 200 |
